...
class Wf_Woocommerce_Packing_List {
	...
	private function load_dependencies() {
		 ...
		 * core plugin.
		 */
		require_once plugin_dir_path( __DIR__ ) . 'includes/class-wf-woocommerce-packing-list-loader.php';

		/**
		 * The class responsible for defining internationalization functionality
		 * of the plugin.
		 */
		require_once plugin_dir_path( __DIR__ ) . 'includes/class-wf-woocommerce-packing-list-i18n.php';

		/**
		 * The class responsible for defining all actions that occur in the admin area.
		 */
		require_once plugin_dir_path( __DIR__ ) . 'admin/class-wf-woocommerce-packing-list-admin.php';

		require_once plugin_dir_path( __DIR__ ) . 'includes/sequential-number.php';

		require_once plugin_dir_path( __DIR__ ) . 'includes/class-wf-woocommerce-packing-list-order-func.php';
		/**
		 * The class responsible for defining all actions that occur in the public-facing
		 * side of the site.
		 */
		require_once plugin_dir_path( __DIR__ ) . 'public/class-wf-woocommerce-packing-list-public.php';

		require_once plugin_dir_path( __DIR__ ) . 'includes/class-wf-woocommerce-packing-list-update-install.php';

		/**
		 * The class responsible for defining all actions that occur in common for public and admin -facing
		 * side of the site.
		 */
		require_once plugin_dir_path( __DIR__ ) . 'common/class-wt-pklist-common.php';

		/**
		 * Includes review request class file
		 */
		require_once plugin_dir_path( __DIR__ ) . 'includes/class-wf-woocommerce-packing-list-review_request.php';

		/**
		 * Details of pro addons list
		 */
		require_once plugin_dir_path( __DIR__ ) . 'includes/class-wf-woocommerce-packing-list-pro-addons.php';

		/**
		 * Includes review request class file
		 */
		require_once plugin_dir_path( __DIR__ ) . 'includes/admin/class-wf-woocommerce-packing-list-admin_notices.php';

		/**
		 * Includes the CTA banners for smart coupon, Pdf invoice and product import export for woocommerce
		 */
		require_once plugin_dir_path( __DIR__ ) . 'admin/modules/cross-promotion-banners/class-wbte-cross-promotion-banners.php';

		require_once plugin_dir_path( __DIR__ ) . 'admin/modules/banner/class-wt-mpdf-language-banner.php';

		/**
		 * Includes the GDPR CTA banner
		 */
		require_once plugin_dir_path( __DIR__ ) . 'admin/modules/banner/class-wt-gdpr-cta-banner.php';

		/**
		 * Includes the Milestone and Loyalty promotional CTA banners
		 */
		 ...
		 */
		require_once plugin_dir_path( __DIR__ ) . 'admin/modules/wt-pdf-request-feature/class-wt-pdf-request-feature.php';

		include plugin_dir_path( __DIR__ ) . 'admin/views/_form_field_generator_new.php';
	...
	public function define_admin_basic_common_func_hooks() {
		...
		if ( isset( $_GET['page'] ) && in_array( $_GET['page'], $invoice_page_js_arr ) && ! class_exists( 'Wf_Woocommerce_Packing_List_Pro_Common_Func' ) ) { // phpcs:ignore WordPress.Security.NonceVerification.Recommended -- This is a safe use of isset.
			require_once plugin_dir_path( __DIR__ ) . 'includes/class-wf-woocommerce-packing-list-basic-func.php';
	...
	public static function generate_print_button_for_user( $order, $order_id, $action, $label, $email_button = false ) {
		if ( ! empty( $order ) ) {
			$document_link = add_query_arg(
				array(
					'attaching_pdf'     => 1,
					'print_packinglist' => 'true',
					'email'             => self::wf_encode( $order->get_billing_email() ),
	...
	public static function get_display_key( $meta_key ) {
		...
		} elseif ( ( 'cus_note' === $meta_key ) || in_array( $meta_key, self::$default_fields_no_prefix_vat ) ) {
			$meta_key_display = '';
		} elseif ( 'aelia_vat' === $meta_key ) {
	...
	/**
	 * Get or generate the site-level PDF storage token.
	 * Token is stored in wp_options and shared across all document types for this installation.
	 *
	 * @since 4.9.5
	 * @return string Alphanumeric token (min 32 chars).
	 */
	public static function get_site_pdf_token() {
		$token = get_option( 'wt_pklist_site_token', '' );
		if ( empty( $token ) || ! preg_match( '/^[a-zA-Z0-9_-]{32,}$/', $token ) ) {
			$token = wp_generate_password( 32, false );
			update_option( 'wt_pklist_site_token', $token, false );
		}
		return $token;
	}

	/**
	 * Place an index.php silence file in a directory if not already present.
	 *
	 * @since 4.9.5
	 * @param string $dir Absolute path to the directory.
	 */
	public static function ensure_secure_subdir( $dir ) {
		...
		foreach ( $files as $filename => $content ) {
			if ( ! file_exists( $dir . $filename ) ) {
				file_put_contents( $dir . $filename, $content ); // phpcs:ignore WordPress.WP.AlternativeFunctions.file_system_operations_file_put_contents
			}
		}
	}
}