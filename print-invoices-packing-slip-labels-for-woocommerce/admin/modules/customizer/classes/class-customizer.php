...
class Wf_Woocommerce_Packing_List_CustomizerLib {
	...
	public static function set_extra_charge_fields( $find_replace, $template_type, $html, $order = null ) {
		...
		if ( ! is_null( $order ) ) {
			...
			// order discount ==========================
			$order_discount = Wt_Pklist_Common::get_order_meta( $order_id, '_order_discount', true );
			if ( $order_discount > 0 ) {
			...
			if ( ! isset( $find_replace['[wfte_product_table_payment_total]'] ) || ! isset( $find_replace['[wfte_total_in_words]'] ) ) {
				$total_price_final = (float) Wt_Pklist_Common::get_order_meta( $order_id, 'total', true );
	...
	*/
	private static function generate_product_table_product_row_html( $columns_list_arr, $template_type, $order = null, $column_list_options_value = array() ) {
		...
		if ( ! is_null( $order ) ) {
			...
			foreach ( $order_items as $order_item_id => $order_item ) {
				...
				$product_total_free_order = $order->get_line_subtotal( $order_item, $incl_tax, true );
				...
				} elseif ( 'No' === $free_line_items_enable ) {
					if ( ( 0.0 === (float) $order_item['line_total'] ) || ( 0.0 === (float) $product_total_free_order ) ) {
						continue;
					}
				}
					...
					foreach ( $columns_list_arr as $columns_key => $columns_value ) {
						...
						if ( 'serial_no' === $columns_key || '-serial_no' === $columns_key ) {
							...
							*/
							...
							$product_total = $order->get_line_subtotal( $order_item, $incl_tax, true );
	...
	public static function tax_column_in_product_table_row( $module_id, $template_type, $order, $order_item, $order_item_id, $_product = null ) {
		...
		if ( ( 'amount' === $total_tax_column_display_option ) || ( 'amount-rate' === $total_tax_column_display_option ) ) {
			$product_total = (float) $order->get_line_subtotal( $order_item, false, true );
	...
	private static function get_default_order_fields( $key, $template_type, $order, $order_id ) {
	...
	public static function set_default_order_fields( $find_replace, $template_type, $html, $order = null ) {
		if ( ! is_null( $order ) ) {
			...
			$order_id                             = $order->get_id();
			$find_replace['[wfte_email]']         = self::get_default_order_fields( 'email', $template_type, $order, $order_id );
			$find_replace['[wfte_tel]']           = self::get_default_order_fields( 'tel', $template_type, $order, $order_id );
			$find_replace['[wfte_customer_note]'] = self::get_default_order_fields( 'customer_note', $template_type, $order, $order_id );
			$find_replace['[wfte_ssn_number]']    = self::get_default_order_fields( 'ssn', $template_type, $order, $order_id );
			$find_replace['[wfte_vat_number]']    = self::get_default_order_fields( 'vat', $template_type, $order, $order_id );
		}
		return $find_replace;
	}
	...
	public static function set_extra_fields( $find_replace, $template_type, $html, $order = null ) {
		...
		if ( ! is_null( $order ) ) {
			...
			$extra_fields['cus_note'] = self::get_default_order_fields( 'customer_note', $template_type, $order, $order_id );
			$extra_fields['vat']      = self::get_default_order_fields( 'vat', $template_type, $order, $order_id );
			$extra_fields['ssn']      = self::get_default_order_fields( 'ssn', $template_type, $order, $order_id );
	...
	public static function set_shipping_from_address( $find_replace, $template_type, $order = null ) {
		...
		$fromaddress      = array(
			...
			'email'          => isset( $the_options['woocommerce_wf_packinglist_sender_email'] ) ? $the_options['woocommerce_wf_packinglist_sender_email'] : '',
		...
		if(!has_filter('wf_pklist_alter_shipping_from_address')){ 
			...
			if ( ! empty( $the_options['woocommerce_wf_packinglist_sender_email'] ?? '' ) ) {
				$find_replace['[wfte_from_address]'] .= '<br>' . esc_html( $the_options['woocommerce_wf_packinglist_sender_email'] );
			}
		...
		if(!has_filter('wf_pklist_alter_shipping_return_address')){
			...
			if ( ! empty( $the_options['woocommerce_wf_packinglist_sender_email'] ?? '' ) ) {
				$find_replace['[wfte_return_address]'] .= '<br>' . esc_html( $the_options['woocommerce_wf_packinglist_sender_email'] );
			}
		}
