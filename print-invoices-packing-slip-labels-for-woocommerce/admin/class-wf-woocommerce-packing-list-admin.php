...
class Wf_Woocommerce_Packing_List_Admin {
								...
								<div class="wt_pklist_<?php echo esc_attr( $button_type ); ?>_content">
									...
									foreach ( $args['items'] as $btnkk => $btnvv ) {
										...
										if ( 0 !== $is_show_prompt ) {
											...
											} elseif ( false !== strpos( $action, 'print_' ) && false === strpos( $action, 'print_ubl' ) ) {
	...
	public function print_window() {
		if ( isset( $_REQUEST['print_packinglist'] ) ) { // phpcs:ignore WordPress.Security.NonceVerification.Recommended,WordPress.Security.ValidatedSanitizedInput.MissingUnslash,WordPress.Security.ValidatedSanitizedInput.InputNotSanitized reason : guest users also need to print the document from the mail link
			...
			if ( 'logged_in' === $document_access_type ) {
				...
				} elseif ( is_user_logged_in() && isset( $attaching_pdf ) && wp_verify_nonce( $nonce, WF_PKLIST_PLUGIN_NAME ) ) {
					// guest link
	...
	public function print_document_from_the_admin_link() {
		...
		if ( 1 === count( $orders ) ) {
			...
			if ( empty( $order ) ) {
			...
			} elseif ( self::is_order_unsaved( $order ) && self::action_burns_invoice_number( isset( $_GET['type'] ) ? sanitize_text_field( wp_unslash( $_GET['type'] ) ) : '' ) ) { // phpcs:ignore WordPress.Security.NonceVerification.Recommended -- nonce already verified earlier in this handler.
				// Block auto-draft orders from burning an invoice/proforma number before they're saved.
				self::wt_pklist_safe_redirect_or_die( null, __( 'The order has not been created yet. Please save the order first before generating an invoice.', 'print-invoices-packing-slip-labels-for-woocommerce' ) );
			} elseif ( ! self::check_role_access() ) {
				...
				} elseif ( 0 === $user_id ) { // Guest user is not allowed to print/download through the admin link
	...
	private function dismiss_notice() {
		...
		if ( isset( $_GET['wf_pklist_notice_dismiss'] ) && '' !== trim( $_GET['wf_pklist_notice_dismiss'] ) ) { // phpcs:ignore WordPress.Security.NonceVerification.Recommended,WordPress.Security.ValidatedSanitizedInput.MissingUnslash,WordPress.Security.ValidatedSanitizedInput.InputNotSanitized
			...
			if ( in_array( $wf_pklist_notice_dismiss, $allowd_items ) ) {
	...
	public function validate_settings_data( $val, $key, $validation_rule = array() ) {
		if ( isset( $validation_rule[ $key ] ) && is_array( $validation_rule[ $key ] ) ) {
			if ( isset( $validation_rule[ $key ]['type'] ) ) {
				...
				} elseif ( 'email' === $validation_rule[ $key ]['type'] ) {
	...
	public function save_settings() {
		...
		if ( self::check_write_access() ) {
			...
			$validation_rule = array(
				...
				'woocommerce_wf_packinglist_sender_email'  => array( 'type' => 'email' ),
					...
					if ( '' !== trim( $_POST['wt_pklist_new_custom_field_title'] ) && '' !== trim( $_POST['wt_pklist_new_custom_field_key'] ) ) { // phpcs:ignore WordPress.Security.NonceVerification.Missing,WordPress.Security.ValidatedSanitizedInput.MissingUnslash,WordPress.Security.ValidatedSanitizedInput.InputNotSanitized
						...
						if ( 'order_meta' === $custom_field_type || 'product_meta' === $custom_field_type ) {
							...
							if ( $can_add_item ) {
								...
								$out = array(
									'key'                => $new_meta_key,
									'val'                => $new_meta_vl . $new_meta_key_display,
									'dc_slug'            => $dc_slug,
									'success'            => true,
									'action'             => $action,
									'old_meta_key'       => $old_meta_key,
									'old_meta_key_label' => $old_meta_key_label,
									'new_meta_label'     => $new_meta_vl,
								);
							} else {
	...
	public function wt_pklist_action_scheduler_for_invoice_number_count() {
		...
		} elseif ( as_next_scheduled_action( 'update_empty_invoice_number_count', array(), $group ) === true ) {
	...
	public function wt_pklist_action_scheduler_for_invoice_number() {
		...
		} elseif ( as_next_scheduled_action( 'wt_pklist_schedule_auto_generate_invoice_number', array(), $group ) === true && false === $data['invoice_enabled'] ) {
	...
	public static function check_before_auto_generating_invoice_no() {
		...
		} elseif ( 'edit.php' === $pagenow && ( isset( $_GET['post_type'] ) && 'shop_order' === $_GET['post_type'] ) ) { // phpcs:ignore WordPress.Security.NonceVerification.Recommended
		...
		if ( $update_count ) {
			if ( false !== get_option( 'wt_created_document_count' ) ) {
				...
				if ( 'invoice' === $template_type ) {
					...
					$daily_data = get_option(
						'wt_pklist_daily_invoice_count',
						array(
							'date'  => '',
							'count' => 0,
						)
					...
					$today      = gmdate( 'Y-m-d' );
					if ( is_array( $daily_data ) && isset( $daily_data['date'] ) && $daily_data['date'] === $today ) {
					...
					update_option( 'wt_pklist_daily_invoice_count', $daily_data );
				}
			} else {
		...
		if ( ! self::check_write_access() ) {
			echo json_encode(
				array(
					'status'  => true,
					'doc_set' => 0,
					'message' => __( 'You are not allowed to do this action', 'print-invoices-packing-slip-labels-for-woocommerce' ),
				)
			);
			exit();
		}
	...
	public function wt_pklist_popup_on_order_edit_page() {
		...
		if ( true === $show_popup ) {
			$data = '<div class="wt_doc_create_confirm_popup wf_pklist_popup" style="width:40%;text-align:left;">
					<div style="padding:20px 25px; width:100%; box-sizing:border-box; overflow:hidden;">
	...
	public function wt_pklist_cta_banner_dismiss() {
		...
		if ( ! empty( $banner_class ) ) {
			...
			$result = array(
				'status'  => true,
				'message' => __( 'Success', 'print-invoices-packing-slip-labels-for-woocommerce' ),
			);
		} else {
			$result = array(
				'status'  => false,
				'message' => __( 'Cannot close this banner', 'print-invoices-packing-slip-labels-for-woocommerce' ),
			);
		}
		echo json_encode( $result );
		exit();
	...
	public function wt_pklist_settings_json() {
		...
		if ( ! wp_verify_nonce( $export_module_nonce, WF_PKLIST_PLUGIN_NAME . '_debug_export_form' ) || ! self::check_role_access() ) {
		...
		if ( self::check_role_access() ) {
			...
			$response = array(
				'wt_pklist_key'          => 'wt_pklist',
				'options'                => array(),
				'wfpklist_template_data' => array(),
			);
			if ( is_array( $options ) ) {
	...
	public function wt_pklist_import_settings() {
		if ( isset( $_POST['wt_pklist_settings_import_confirm_text'] ) ) {
			if ( 'confirm' === $_POST['wt_pklist_settings_import_confirm_text'] ) { // phpcs:ignore WordPress.Security.NonceVerification.Missing
				...
				if ( ! wp_verify_nonce( $import_module_nonce, WF_PKLIST_PLUGIN_NAME . '_debug_import_form' ) || ! self::check_role_access() ) {
					self::wt_pklist_safe_redirect_or_die( null, __( 'You are not allowed to do this action', 'print-invoices-packing-slip-labels-for-woocommerce' ) );
				} else {
	...
	public function wt_pklist_reset_settings() {
		...
		if ( isset( $_POST['wt_pklist_settings_reset_confirm_text'] ) ) {
			...
			if ( ! wp_verify_nonce( $reset_nonce, WF_PKLIST_PLUGIN_NAME . '_debug_reset_form' ) || ! self::check_role_access() ) { // added nonce verification and capability check
	...
	public function wt_pklist_action_scheduler_for_saving_default_templates() {
		...
		} elseif ( as_next_scheduled_action( 'wt_pklist_save_default_templates', array(), $group ) === true ) {
		...
		if ( is_dir( $backup_dir ) ) {
			...
			foreach ( $files as $name => $file ) {
				if ( ! $file->isDir() ) {
					...
					$file_ext_arr = explode( '.', $file_name );
					$file_ext     = end( $file_ext_arr );
					if ( ( $file_ext === 'pdf' ) || ( $file_ext === 'zip' ) || ( $file_ext === 'html' ) ) {
						wp_delete_file( $file );
					}
				}
			}
	...
	public function wt_pklist_action_scheduler_for_auto_cleanup() {
		...
		if ( 'Yes' === $is_auto_clear && $is_auto_clear_interval > 0 ) {
	...
	public function wt_pklist_form_wizard_save() {
		...
		if ( self::check_write_access() ) {
			...
			$general_module_fields = array(
				...
				'woocommerce_wf_packinglist_sender_email',
	...
	public function update_plugin_settings_with_migration( $options, $base_id ) {
		if ( '' === $base_id ) {
			if ( isset( $options['woocommerce_wf_packinglist_preview'] ) ) {
				...
				} elseif ( 'disabled' === $options['woocommerce_wf_packinglist_preview'] ) {
