	...
	class Invoice extends WtPdfUblGenerator {
		...
		public function generate_ubl_invoice( $order, $action ) {
			...
			if ( ! empty( $order ) ) {
				...
				} elseif ( 'attach_ublinvoice' === $action ) {
					$xmlHandler = new \Wtpdf\Ubl\Handler\XmlFileHandler();
		...
		public function get_shop_name() {
			...
			if ( ! empty( $company_name ) ) {
				return $company_name;
			} elseif ( ! empty( get_option( 'woocommerce_store_name' ) ) ) {
		...
		public function get_address_line() {
			...
			} elseif ( ! empty( get_option( 'woocommerce_store_address' ) ) ) {
		...
		public function get_shop_city() {
			...
			if ( ! empty( $city ) ) {
				return $city;
			} elseif ( ! empty( get_option( 'woocommerce_store_city' ) ) ) {
		...
		public function get_shop_postalcode() {
			...
			if ( ! empty( $city ) ) {
				return $city;
			} elseif ( ! empty( get_option( 'woocommerce_store_postcode' ) ) ) {
		...
		public function get_shop_country() {
			...
			if ( ! empty( $country ) ) {
				return $country;
			} elseif ( ! empty( get_option( 'woocommerce_default_country' ) ) ) {
